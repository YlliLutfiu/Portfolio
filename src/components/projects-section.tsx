...
export default function ProjectsSection({}: ProjectsSectionProps) {
  return (
    ...
    >
      <div className="grid auto-rows-[1fr] gap-7 md:grid-cols-2 lg:grid-cols-6">
        {projects.map((project: Project, index: number) => (
          <motion.article
            ...
            className={`group premium-card relative overflow-hidden rounded-3xl p-6 lg:p-7 ${
              index === 0 ? 'md:col-span-2 lg:col-span-3 xl:col-span-4' : 'md:col-span-1 lg:col-span-2'
            }`}
          >
            ...
            <div className="relative flex h-full flex-col">
              ...
              {project.images && project.images.length > 0 && (
                <div className="relative mb-5 bg-transparent aspect-[16/10] w-full rounded-2xl overflow-hidden border-none shadow-2xl">
                  <Swiper
                    ...
                    className="[&_.swiper-slide]:h-full [&_.swiper-slide]:flex [&_.swiper-slide]:items-center [&_.swiper-slide]:justify-center h-full w-full"
                  >
                  ...
                  <button
                    type="button"
                    className={`project-${index}-prev absolute left-3 top-1/2 -translate-y-1/2 z-10 flex h-12 w-12 items-center justify-center rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-accent hover:bg-accent/20 transition-all duration-300 opacity-80 hover:opacity-100 group-hover:left-4 md:left-4`}
                  >
                    <ChevronLeft className="h-5 w-5" />
                  </button>
                  <button
                    type="button"
                    className={`project-${index}-next absolute right-3 top-1/2 -translate-y-1/2 z-10 flex h-12 w-12 items-center justify-center rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-accent hover:bg-accent/20 transition-all duration-300 opacity-80 hover:opacity-100 group-hover:right-4 md:right-4`}
                  >
                    <ChevronRight className="h-5 w-5" />
                  </button>
                </div>
              )}
              <p className="mt-4 text-sm leading-7 text-foreground/74">{project.description}</p>
